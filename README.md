...
The harvest process is triggered by messages from RabbitMQ with the routing key `dataservice.*.HarvestTrigger`, a
...
will be ignored. And then find all data services each catalog contains, indicated by the predicate `dcat:service` and
type `dcat:DataService`, blank node data services will be ignored.
When all catalogs and data services have been found a recursive function will create a graph with every contained triple
for all catalogs and data services.
...
`dataservices.harvested`, the message body will be a list of harvest reports, one report for each source from
fdk-harvest-admin.
